...
_DEMO_EMAIL    = "demo@stockcast.com"
_DEMO_PASSWORD = "demo1234"
_DEMO_USER_ID  = "00000000-0000-0000-0000-000000000000"

class _DemoUser:
    """Minimal user object compatible with _user_email() and _user_id() in app.py."""
    email = _DEMO_EMAIL
    id    = _DEMO_USER_ID

class _DemoSession:
    """Wraps _DemoUser to match supabase-py ≥2.0 Session shape."""
    user = _DemoUser()
...
def render_auth_gate(supabase):
    ...
    if mode == "login":
        ...
        st.markdown("""
        <div class="sc-demo-banner">
            👁&nbsp; <b>Try without an account</b> — read-only demo, no data saved
        </div>
        """, unsafe_allow_html=True)

        st.markdown('<div class="sc-demo-btn">', unsafe_allow_html=True)
        if st.button("▷  Enter Demo Mode", use_container_width=True, key="demo_btn"):
            st.session_state.user      = _DemoSession()
            st.session_state.user_plan = "free"
            st.session_state["_is_demo"] = True
            st.success("Demo mode active — explore freely!")
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown("<div style='display:flex;align-items:center;gap:.5rem;margin:.8rem 0;'>"
                    "<div style='flex:1;height:1px;background:#1e2d45;'></div>"
                    "<span style='font-size:.68rem;color:#3e4558;font-family:var(--font-mono);'>or sign in</span>"
    ...
    <div class="sc-footer">
        ...
        <span class="mono">StockCast v1.0 · © 2025 All rights reserved</span>
    </div>
    """, unsafe_allow_html=True)
