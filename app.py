...
if st.session_state.user is None:
    ...
    html, body, [data-testid="stApp"], [data-testid="stAppViewContainer"], .main {
        background:
            radial-gradient(ellipse at 15% 25%, rgba(77,142,255,0.12), transparent 45%),
            radial-gradient(ellipse at 85% 75%, rgba(173,198,255,0.08), transparent 45%),
            #0b1326 !important;
    }
    .block-container { padding: 0 !important; max-width: 100% !important; }
    header[data-testid="stHeader"], footer, #MainMenu { display: none !important; }
    [data-testid="stSidebar"] { display: none !important; }
    @keyframes floatBlob1 { 0%,100%{transform:translate(0,0) scale(1);} 50%{transform:translate(40px,30px) scale(1.06);} }
    @keyframes floatBlob2 { 0%,100%{transform:translate(0,0) scale(1);} 50%{transform:translate(-30px,-20px) scale(1.04);} }
    @keyframes cardSlide  { from{opacity:0;transform:translateY(20px);} to{opacity:1;transform:translateY(0);} }
    </style>
    """, unsafe_allow_html=True)

    _l, _mid, _r = st.columns([1, 1.3, 1])
    with _mid:
        st.markdown("""
        <div style="animation:cardSlide 0.5s ease both;background:rgba(19,27,46,0.90);
             border:1px solid rgba(173,198,255,0.12);border-top:1px solid rgba(77,142,255,0.3);
             border-radius:1.25rem;padding:2.5rem 2.2rem 2rem;
             box-shadow:0 24px 64px rgba(0,0,0,0.7),0 0 60px rgba(77,142,255,0.06);
             margin-top:3rem;">
          <div style="text-align:center;margin-bottom:1.8rem;">
            <div style="display:inline-flex;align-items:center;justify-content:center;
                 width:56px;height:56px;background:linear-gradient(135deg,rgba(77,142,255,0.2),rgba(77,142,255,0.05));
                 border:1px solid rgba(77,142,255,0.35);border-radius:14px;
                 font-size:1.6rem;margin-bottom:1rem;
                 box-shadow:0 0 28px rgba(77,142,255,0.2);">📈</div>
            <div style="font-family:Manrope,sans-serif;font-size:2rem;font-weight:800;
                 color:#dae2fd;letter-spacing:-.02em;">
              Wealth<span style="color:#4d8eff;">Intel</span>
            </div>
            <div style="font-size:.8rem;color:#424754;margin-top:.4rem;letter-spacing:.04em;">
        ...
        with auth_tab2:
            ...
            signup_confirm = st.text_input("Confirm Password", key="signup_confirm", type="password", placeholder="Re-enter password")
        ...
        st.markdown("""
          <div style="text-align:center;margin-top:1.4rem;display:flex;justify-content:center;gap:1.4rem;flex-wrap:wrap;">
            <span style="font-size:.68rem;color:#424754;display:flex;align-items:center;gap:.3rem;">
              <span style="color:#4d8eff;">✓</span> 256-bit encryption</span>
            <span style="font-size:.68rem;color:#424754;display:flex;align-items:center;gap:.3rem;">
              <span style="color:#4d8eff;">✓</span> No data stored</span>
            <span style="font-size:.68rem;color:#424754;display:flex;align-items:center;gap:.3rem;">
              <span style="color:#4d8eff;">✓</span> Free to use</span>
          </div>
          <div style="text-align:center;margin-top:1.2rem;font-size:.58rem;color:#2d3449;letter-spacing:.08em;">
