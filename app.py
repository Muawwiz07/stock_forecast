...
if st.session_state.user is None:
    ...
    [data-testid="stTextInput"] input::placeholder { color: rgba(117,117,118,0.5) !important; }
    [data-testid="stTextInput"] label { display: none !important; }

    /* Tabs */
    [data-testid="stTabs"] [role="tablist"] {
        background: transparent !important;
        border-bottom: 1px solid rgba(71,72,73,0.1) !important;
        gap: 2rem !important;
    }
    [data-testid="stTabs"] [role="tab"] {
    ...
    [data-testid="stTabs"] [role="tab"][aria-selected="true"] {
        ...
        border-bottom: 2px solid #3fff8b !important;
        background: transparent !important;
    }
    [data-testid="stTabPanel"] { background: transparent !important; padding: 1.5rem 0 0 !important; }
    ...
    .stButton > button {
        ...
        box-shadow: 0 8px 24px rgba(63,255,139,0.15) !important;
        transition: transform 0.15s, box-shadow 0.15s !important;
    }
    .stButton > button:hover {
        transform: translateY(-1px) !important;
        box-shadow: 0 12px 32px rgba(63,255,139,0.25) !important;
    }
    ...
    /* Forecast bars animation */
    @keyframes sc-rise { from{height:0;} to{height:var(--h);} }
    </style>
    """, unsafe_allow_html=True)

    # ── Full-page layout: left brand + right form ─────────────────────────────
    left_col, right_col = st.columns([1, 1], gap="small")

    # ── LEFT: Brand Panel ──────────────────────────────────────────────────────
    with left_col:
        st.markdown("""
        <div style="background:#121315;min-height:100vh;padding:3rem;display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden;">

          <!-- Dot-grid texture -->
          <div style="position:absolute;inset:0;background-image:radial-gradient(circle at 2px 2px,rgba(63,255,139,0.05) 1px,transparent 0);background-size:24px 24px;pointer-events:none;"></div>

          <!-- Ambient glow -->
          <div style="position:absolute;top:25%;left:-5rem;width:24rem;height:24rem;background:rgba(63,255,139,0.07);filter:blur(80px);border-radius:50%;pointer-events:none;"></div>
          <div style="position:absolute;bottom:25%;right:-5rem;width:24rem;height:24rem;background:rgba(69,254,201,0.04);filter:blur(80px);border-radius:50%;pointer-events:none;"></div>

          <!-- Content -->
          <div style="position:relative;z-index:1;">
            <!-- Logo -->
            <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:3rem;">
              <span class="material-symbols-outlined" style="color:#3fff8b;font-size:2.2rem;font-variation-settings:'FILL' 1;">insights</span>
              <h1 style="font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:1.6rem;letter-spacing:-0.02em;color:#3fff8b;margin:0;">Stockcast Terminal</h1>
            </div>

            <!-- Tagline -->
            <h2 style="font-family:'Space Grotesk',sans-serif;font-size:2.4rem;font-weight:500;line-height:1.2;color:#ffffff;margin-bottom:1.2rem;">Predicting the pulse of<br>global markets.</h2>
            <p style="font-family:'Inter',sans-serif;font-size:1rem;color:#ababac;line-height:1.7;max-width:400px;">
              Access high-frequency forecasting models and neural-driven market analysis in one unified interface.
            </p>
          </div>

          <!-- Live Alpha Stream Card -->
          <div style="position:relative;z-index:1;background:#252627;padding:1.5rem;border-radius:0.25rem;border:1px solid rgba(71,72,73,0.15);">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
              <span style="font-family:'Space Grotesk',sans-serif;font-size:0.65rem;letter-spacing:0.12em;text-transform:uppercase;color:#ababac;">Live Alpha Stream</span>
              <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#3fff8b;animation:sc-pulse 2s infinite;box-shadow:0 0 8px rgba(63,255,139,0.5);"></span>
            </div>

            <!-- Abstract chart bars -->
            <div style="height:8rem;background:#000000;border-radius:0.125rem;overflow:hidden;display:flex;align-items:flex-end;padding:0.5rem 0.5rem 0.5rem;gap:0.25rem;">
              <div style="background:rgba(63,255,139,0.2);flex:1;height:40%;border-radius:2px 2px 0 0;"></div>
              <div style="background:rgba(63,255,139,0.4);flex:1;height:60%;border-radius:2px 2px 0 0;"></div>
              <div style="background:rgba(63,255,139,0.6);flex:1;height:55%;border-radius:2px 2px 0 0;"></div>
              <div style="background:rgba(63,255,139,0.8);flex:1;height:85%;border-radius:2px 2px 0 0;"></div>
              <div style="background:#3fff8b;flex:1;height:100%;border-radius:2px 2px 0 0;box-shadow:0 0 15px rgba(63,255,139,0.3);"></div>
            </div>

            <div style="margin-top:1rem;display:flex;justify-content:space-between;">
              <span style="font-family:'Space Grotesk',sans-serif;font-size:0.75rem;color:#ababac;">Model Confidence</span>
              <span style="font-family:'Space Grotesk',sans-serif;font-size:0.75rem;font-weight:700;color:#3fff8b;">98.4%</span>
            </div>
          </div>
    ...
    with right_col:
        st.markdown("""
        <div style="background:#0d0e0f;min-height:100vh;padding:4rem 4rem 2rem;">
        """, unsafe_allow_html=True)

        auth_tab1, auth_tab2 = st.tabs(["Terminal Access", "Create Account"])

        with auth_tab1:
            st.markdown("""
            <div style="margin-bottom:1.5rem;">
              <h3 style="font-family:'Space Grotesk',sans-serif;font-size:1.5rem;font-weight:700;color:#ffffff;margin-bottom:0.3rem;">Welcome Back</h3>
              <p style="font-family:'Inter',sans-serif;font-size:0.85rem;color:#ababac;">Synchronize with the terminal to resume analysis.</p>
            </div>
            <div style="margin-bottom:0.4rem;">
              <span style="font-family:'Space Grotesk',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#ababac;">Identity Token (Email)</span>
            </div>
            """, unsafe_allow_html=True)
            login_email = st.text_input("Email", key="login_email", placeholder="name@firm.com")

            st.markdown("""
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.4rem;margin-top:0.75rem;">
              <span style="font-family:'Space Grotesk',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#ababac;">Access Key</span>
              <a href="#" style="font-size:0.62rem;color:#3fff8b;text-decoration:none;text-transform:uppercase;letter-spacing:0.05em;">Forgot Key?</a>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown("""
            <div style="position:relative;padding:1.2rem 0;">
              <div style="position:absolute;inset:0;display:flex;align-items:center;">
                <div style="width:100%;border-top:1px solid rgba(71,72,73,0.1);"></div>
              </div>
              <div style="position:relative;display:flex;justify-content:center;">
                <span style="background:#0d0e0f;padding:0 1rem;font-family:'Space Grotesk',sans-serif;font-size:0.65rem;text-transform:uppercase;letter-spacing:0.1em;color:#757576;">Alternative Protocols</span>
              </div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:2rem;">
              <button style="display:flex;align-items:center;justify-content:center;gap:0.5rem;padding:0.75rem;background:#1e2021;border:1px solid rgba(71,72,73,0.1);border-radius:0.5rem;color:#ffffff;font-family:'Inter',sans-serif;font-size:0.75rem;font-weight:500;cursor:pointer;">
                <svg width="16" height="16" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                Google
              </button>
              <button style="display:flex;align-items:center;justify-content:center;gap:0.5rem;padding:0.75rem;background:#1e2021;border:1px solid rgba(71,72,73,0.1);border-radius:0.5rem;color:#ffffff;font-family:'Inter',sans-serif;font-size:0.75rem;font-weight:500;cursor:pointer;">
                <span class="material-symbols-outlined" style="font-size:1.1rem;">terminal</span>
                SSO
              </button>
            </div>

            <!-- Limited Offer Banner -->
            <div style="padding:1rem;background:rgba(63,255,139,0.05);border:1px solid rgba(63,255,139,0.1);border-radius:0.5rem;display:flex;align-items:flex-start;gap:0.75rem;">
              <span class="material-symbols-outlined" style="color:#3fff8b;font-size:1.3rem;font-variation-settings:'FILL' 1;flex-shrink:0;">verified</span>
              <div>
                <p style="font-family:'Space Grotesk',sans-serif;font-size:0.65rem;font-weight:700;color:#3fff8b;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.25rem;">Limited Offer</p>
                <p style="font-family:'Inter',sans-serif;font-size:0.75rem;color:#ababac;line-height:1.5;">Start your 14-day trial of Alpha-Tier models. No credit card required for initial calibration.</p>
              </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with auth_tab2:
            st.markdown("""
            <div style="margin-bottom:1.5rem;">
              <h3 style="font-family:'Space Grotesk',sans-serif;font-size:1.5rem;font-weight:700;color:#ffffff;margin-bottom:0.3rem;">Initialize Account</h3>
              <p style="font-family:'Inter',sans-serif;font-size:0.85rem;color:#ababac;">Register your credentials to access the terminal.</p>
            </div>
            <div style="margin-bottom:0.4rem;">
              <span style="font-family:'Space Grotesk',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#ababac;">Identity Token (Email)</span>
            </div>
            """, unsafe_allow_html=True)
            signup_email = st.text_input("Email", key="signup_email", placeholder="name@firm.com")

            st.markdown("""<div style="margin-bottom:0.4rem;margin-top:0.75rem;"><span style="font-family:'Space Grotesk',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#ababac;">Access Key</span></div>""", unsafe_allow_html=True)
            signup_password = st.text_input("Password", key="signup_password", type="password", placeholder="Min. 6 characters")

            st.markdown("""<div style="margin-bottom:0.4rem;margin-top:0.75rem;"><span style="font-family:'Space Grotesk',sans-serif;font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;color:#ababac;">Confirm Access Key</span></div>""", unsafe_allow_html=True)
            signup_confirm = st.text_input("Confirm Password", key="signup_confirm", type="password", placeholder="Re-enter key")

            if st.button("Initialize Account", key="signup_btn"):
                if signup_email and signup_password and signup_confirm:
                    if signup_password != signup_confirm:
                        st.error("⚠ Access keys do not match.")
                    elif len(signup_password) < 6:
                        st.error("⚠ Access key must be at least 6 characters.")
                    else:
                        with st.spinner("Initializing account..."):
                            try:
                                res = supabase.auth.sign_up({"email": signup_email, "password": signup_password})
                                if res.user:
                                    st.success("✓ Account initialized. Check your email to confirm, then log in.")
                                else:
        ...
        <!-- Footer -->
        <div style="background:#000000;padding:3rem 2rem;border-top:1px solid #252627;margin-top:2rem;">
          <div style="max-width:1200px;margin:0 auto;display:flex;flex-direction:column;gap:1.5rem;align-items:center;">
            <div style="display:flex;flex-direction:column;align-items:center;gap:0.5rem;">
              <span style="font-family:'Space Grotesk',sans-serif;font-weight:700;color:#3fff8b;font-size:1.1rem;">Stockcast Terminal</span>
              <p style="font-family:'Inter',sans-serif;font-size:0.72rem;color:#ababac;">© 2024 Stockcast Terminal. All rights reserved.</p>
            </div>
            <div style="display:flex;gap:2rem;flex-wrap:wrap;justify-content:center;">
              <a href="#" style="font-family:'Inter',sans-serif;font-size:0.72rem;color:#ababac;text-decoration:none;">Terms of Service</a>
              <a href="#" style="font-family:'Inter',sans-serif;font-size:0.72rem;color:#ababac;text-decoration:none;">Privacy Policy</a>
              <a href="#" style="font-family:'Inter',sans-serif;font-size:0.72rem;color:#ababac;text-decoration:none;">API Documentation</a>
              <a href="#" style="font-family:'Inter',sans-serif;font-size:0.72rem;color:#ababac;text-decoration:none;">Support</a>
            </div>
            <div style="font-family:'Inter',sans-serif;font-size:0.62rem;color:#474849;letter-spacing:0.08em;text-align:center;">
